...
class ToC3DLightFlexiViTReturnType:
    def __init__(
        self,
        img_feats = None,
        adaptive_patch_size = None,
        token_masks_pre = None,
        attn_scores_pre = None,
        token_masks = None,
        attn_scores = None,
        keep_idx = None,
        drop_idx = None,
        ...
        self.token_masks_pre = token_masks_pre
        self.attn_scores_pre = attn_scores_pre
        self.token_masks = token_masks
        self.attn_scores = attn_scores
        self.keep_idx = keep_idx
        self.drop_idx = drop_idx
...
class LIGHTFlexiEntropyViTReturnType:
    def __init__(
        self,
        img_feats = None,
        token_masks = None,
        ...
        self.align_sourse = align_sourse
...
def batch_index_fill(x, x1, x2, idx1, idx2):
    ...
    x[idx1.reshape(-1)] = x1.reshape(B*N1, C).to(x.dtype)
    x[idx2.reshape(-1)] = x2.reshape(B*N2, C).to(x.dtype)
...
class ScoreBasedTokenSelector(TokenSelectorBase):
    ...
    def sample(self, pred_score: torch.Tensor, override_ratio=None):
        ...
        drop_score = sorted_score[:, num_keep_node:] # [12, 211]
        drop_idx = sorted_idx[:, num_keep_node:] #  [12, 211]
    ...
    def sample_soft(self, pred_score: torch.Tensor, override_ratio=None):
        ratio = override_ratio if override_ratio is not None else self.ratio
        if len(pred_score.shape) == 4:
            pred_score = pred_score.flatten(1, 2)
        if len(pred_score.shape) == 3:
        ...
        # adaptive
        mean_score = score.mean(dim=1, keepdim=True)  # B x 1
        threshold_score = mean_score * 1.0
        B, N = score.shape[:2]
        keep_idx = score >= threshold_score
        num_keep_node = keep_idx.sum(dim=1)
        min_keep_node = num_keep_node.min().item()
        sorted_score, sorted_idx = torch.sort(score, dim=1, descending=True)
        keep_score = sorted_score[:, :min_keep_node]
        keep_idx = sorted_idx[:, :min_keep_node]
        drop_score = sorted_score[:, min_keep_node:]
        drop_idx = sorted_idx[:, min_keep_node:]
...
class SelectiveModule(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.version == 1:
            selector = torch.cat((torch.ones(B, 1, 1, device=selector.device), selector), dim=1).squeeze(2)
        else:  # self.version = 0
            selector = selector.squeeze(2)

        return selector

class NaiveQueryGuidedTokenSelector(ScoreBasedTokenSelector):
    SUPPORTED_SCORE_TYPE = ['attention', 'score', 'entropy', 'cropr', 'svit']
    ...
    def __init__(
        ...
        if self.score_type == 'entropy':
            self.expand = nn.Sequential(
                nn.Linear(128, 2), # dot, sum
                nn.LogSoftmax(dim=-1)
            )
        if self.score_type == 'cropr':
            self.head = nn.Sequential(
                nn.Linear(1, 2), # dot, sum
                nn.LogSoftmax(dim=-1)
            )
        if self.score_type == 'svit':
            self.query_pool = nn.AdaptiveAvgPool1d(1)
            self.svit_selector = SelectiveModule(
                channels=2 * query_dim,
                hidden_channels=2 * query_dim // 4,
                drop=0.1,
                tau=1.0,
                version=0,
    ...
    def query_based_score(self, input_x: torch.Tensor, mask, queries: torch.Tensor, *args, **kwargs):
        ...
        # print(input_x.shape, queries.shape, 333) # [12, 176, 1024], [2, 64, 256]
        input_x = self.input_proj(input_x)  # B x N x C_Q
        queries_embed = queries.repeat_interleave(input_x.shape[0] // queries.shape[0], dim=0)  # B x Q x C_Q [12, 64, 256]

        attention = None
        if self.score_type == 'attention':
            attention = torch.einsum('bnc,bqc->bnq', input_x, queries_embed) * self.scale  # B x N x Q [12, 290, 64]
            pred_score = self.aggregate(attention)  # B x N x 2 [12, 290, 2]
            ...
            pred_score = self.aggregate(x_query) # [12, 29*10, 2]
        elif self.score_type == 'entropy':
            patch_weights = torch.matmul(input_x, queries_embed.transpose(-1, -2))  # [12, 290, 64]
            patch_weights = torch.softmax(patch_weights, dim=-1)  # [12, 290, 64]
            query_weighted = torch.matmul(patch_weights, queries_embed)  # [12, 290, 256]
            fused_features = input_x + query_weighted  # [12, 290, 256]
            fused_features = (fused_features - fused_features.min()) / (fused_features.max() - fused_features.min() + 1e-5)
            fused_features = torch.clamp(fused_features, min=1e-5)

            B, num_patch, dim, chunk_size = fused_features.shape[0], fused_features.shape[1], fused_features.shape[2], 2
            fused_features = fused_features.view(B, num_patch, int(dim / chunk_size), chunk_size)

            entropy = -torch.sum(fused_features * torch.log(fused_features), dim=-1)  # [12, 290]
            pred_score = self.expand(entropy)
        elif self.score_type == 'cropr':
            B, N, C = input_x.shape
            _, Q, _ = queries_embed.shape
            H = 1
            head_dim = C // H
            scale = head_dim ** -0.5

            q = queries_embed.reshape(B, Q, H, head_dim).transpose(1, 2)  # B H Q d
            k = input_x.reshape(B, N, H, head_dim).transpose(1, 2)        # B H N d

            # B H Q N
            attention = torch.matmul(q, k.transpose(-2, -1)) * scale

            # CropR score
            pred_score = attention.sum(dim=(1, 2))  # B x N
            pred_score = self.head(pred_score.unsqueeze(-1))
        elif self.score_type =='svit':
            mean_query_embed = self.query_pool(queries_embed.transpose(1, 2).contiguous()).transpose(1, 2).contiguous()  # B x 1 x C_Q
            mean_query_embed = mean_query_embed.expand_as(input_x).contiguous()
            x_query = torch.concat([input_x, mean_query_embed], dim=-1)  # B x N x (2 * C_Q)
            selector = self.svit_selector(x_query)
            pred_score = self.head(selector.unsqueeze(-1))
...
class MotionAwareQueryGuidedSOFTTokenSelector(NaiveQueryGuidedTokenSelector):
    def __init__(
        self, 
        embed_dim=384, 
        query_dim=256, 
    ...
    def forward(
        ...
        **kwargs
        ...
        score = pred_score[:, :, 0]  # B x (H * W)
        if do_sample:
            keep_score, drop_score, keep_idx, drop_idx, new_mask = self.sample_soft(pred_score, override_ratio)
            # keep_score, drop_score, keep_idx, drop_idx, new_mask = self.sample(pred_score, 0.5)
            new_mask = new_mask.view(B, H, W, 1)
