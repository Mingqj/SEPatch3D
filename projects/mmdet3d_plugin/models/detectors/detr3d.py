...
class Detr3D(MVXTwoStageDetector):
    ...
    def extract_img_feat(
        ...
        GLOBAL_TIMER.event_start('DETR3D/img_neck')
        ...
        if self.training or training_mode:
            for tm in self.position_level:
                BN, C, H, W = img_feats[tm].size()
                img_feat_reshaped = img_feats[tm].view(B, len_queue, int(BN/B / len_queue), C, H, W)
            ...
            for tm in self.position_level:
                BN, C, H, W = img_feats[tm].size()
                img_feat_reshaped = img_feats[tm].view(B, int(BN/B/len_queue), C, H, W)
                img_feats_reshaped.append(img_feat_reshaped)

        GLOBAL_TIMER.event_end('DETR3D/img_neck')
    ...
    def simple_test(self, img_metas, gt_bboxes=None, centers2d=None, depths=None, **data):
        ...
        GLOBAL_TIMER.event_start('DETR3D/3D Transformer')
        ...
        GLOBAL_TIMER.event_end('DETR3D/3D Transformer')
