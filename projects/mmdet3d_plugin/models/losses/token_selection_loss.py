...
@LOSSES.register_module()
class TokenSelectionLoss2(nn.Module):
    def __init__(
        self,
        patch_size = 16,
        semantic_loss = None,
        ...
        super(TokenSelectionLoss2, self).__init__()
        self.patch_size = patch_size
        self.class_weights = class_weights
        self.bg_mask_value = bg_mask_value
        ...
        # if semantic_loss is not None:
        #     self.semantic_target_assigner = TokenSelectionSemanticTargetAssigner(
        #         bg_mask_value=self.bg_mask_value,
        #         patch_size=self.patch_size
        #     )

    def semantic_loss_single_pre(
        self,
        pred_mask: torch.Tensor,
        gt_bboxes: List[torch.Tensor],
        adaptive_patch_size,
        ...
        Returns:
            loss_semantic (torch.Tensor): the semantic loss of a single sample
        '''
        self.patch_size = adaptive_patch_size
        self.semantic_target_assigner = TokenSelectionSemanticTargetAssigner(
                bg_mask_value=self.bg_mask_value,
                patch_size=self.patch_size
            )
        ...
        # print(num_layers, num_views, mask_h, mask_w, mask_labels.shape, 0)
        mask_labels = mask_labels.unsqueeze(0).expand(num_layers, -1, -1, -1).contiguous()
    ...
    def semantic_loss_pre(
        ...
        if self.semantic_loss_func is not None:
            ...
            semantic_loss = self.semantic_loss_single_pre(
                pred_mask, 
                all_gt_bboxes_list, 
                adaptive_patch_size
            )
    ...
    def forward(
        ...
        semantic_loss_pre = self.semantic_loss_pre(pred_mask_pre, gt_bboxes, 16)
        if semantic_loss_pre is not None:
            semantic_loss_pre = torch.nan_to_num(semantic_loss_pre)
            total_loss['semantic_pre_loss'] = semantic_loss_pre

        semantic_loss = self.semantic_loss(pred_mask, gt_bboxes, adaptive_patch_size)
        if semantic_loss is not None:
            semantic_loss = torch.nan_to_num(semantic_loss)
            total_loss['semantic_loss'] = semantic_loss
