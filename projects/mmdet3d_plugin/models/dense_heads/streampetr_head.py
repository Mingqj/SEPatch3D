...
with open("/opt/data/private/codeN1/mmdetection3d/data/fourcam/intrinsic&lidar2camera_extrinsic.json", "r") as f:
    data = json.load(f)

cam_order_intr = ["front_camera-intrinsic", "right_camera-intrinsic", "back_camera-intrinsic", "left_camera-intrinsic"]
cam_order_extr = ["top_center_lidar-to-front_camera-extrinsic", "top_center_lidar-to-right_camera-extrinsic", "top_center_lidar-to-back_camera-extrinsic", "top_center_lidar-to-left_camera-extrinsic"]

intrinsic_global = np.stack([np.array(data[cam]["data"]) for cam in cam_order_intr], axis=0) # (4, 4, 4)
extrinsic = np.stack([np.array(data[key]["data"]) for key in cam_order_extr], axis=0)

lidar2img = []
for item in range(extrinsic.shape[0]):
    lidar2cam_rt = invert_matrix_egopose_numpy(extrinsic[item])
    viewpad = np.eye(4)
    viewpad[:intrinsic_global.shape[0], :intrinsic_global.shape[1]] = intrinsic_global[item]
    lidar2img_rt = (viewpad @ lidar2cam_rt)
    lidar2img.append(lidar2img_rt)
lidar2img_global = np.array(lidar2img)

# ego pose
with open("/opt/data/private/codeN1/mmdetection3d/data/fourcam/ego_poses.json", "r") as f:
    data = json.load(f)
...
for frame in frames:
    pose = np.array(frame["pose"], dtype=np.float32)
    inv_pose = invert_matrix_egopose_numpy(pose)
    
    ego_pose_list.append({
        "frame_id": frame["frame_id"],
        "ego_pose": pose,
        "ego_pose_inv": inv_pose
    })
...
class StreamPETRHead(AnchorFreeHead):
    ...
    def pre_update_memory(self, data):
        ...
        B = x.size(0)
        # #################################################
        # global timei, ego_pose_list
        # data['ego_pose_inv'] = torch.tensor(ego_pose_list[timei]['ego_pose_inv']).float().cuda().unsqueeze(0)
        # # print(data['ego_pose_inv'].shape)
        # #################################################
    ...
    def post_update_memory(self, data, rec_ego_pose, all_cls_scores, all_bbox_preds, outs_dec, mask_dict):
        # ##################################################
        # global timei, timeii, ego_pose_list
        # data['ego_pose'] = torch.tensor(ego_pose_list[timei]['ego_pose']).float().cuda().unsqueeze(0)
        # timei += 1
        # # if timeii % 8 == 0:
        # #     timei += 1
        # ##################################################
    ...
    def position_embeding(self, data, memory_centers, topk_indexes, img_metas):
        ...
        # # data['intrinsics'] = data['intrinsics'][:, 1, :, :].unsqueeze(0) ########
        # # data['lidar2img'] = data['lidar2img'][:, 1, :, :].unsqueeze(0)
        # # # print(data['intrinsics'].shape)

        # global intrinsic_global, lidar2img_global
        # data['intrinsics'] = torch.tensor(intrinsic_global).float().unsqueeze(0).cuda()
        # data['lidar2img'] = torch.tensor(lidar2img_global).float().unsqueeze(0).cuda()
    ...
    def prepare_for_ray_dn(self, batch_size, reference_points, img_metas, data):
        if self.training and self.with_dn:
            targets = [torch.cat((img_meta['gt_bboxes_3d']._data.gravity_center, img_meta['gt_bboxes_3d']._data.tensor[:, 3:]),dim=1) for img_meta in img_metas ]
            labels = [img_meta['gt_labels_3d']._data for img_meta in img_metas ]
            known = [(torch.ones_like(t)).cuda() for t in labels]
    ...
    def forward(
        ...
        **data
        ...
        # reference_points, attn_mask, mask_dict = self.prepare_for_ray_dn(B, reference_points, img_metas, data) # ray dn, only training
