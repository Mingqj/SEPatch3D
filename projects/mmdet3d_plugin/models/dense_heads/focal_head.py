...
class FocalHead(AnchorFreeHead):
    ...
    def loss_single(self,
        ...
        (heatmaps, ) = multi_apply(self._get_heatmap_single, all_centers2d_list, gt_bboxes_list, img_shape, centerness)
    ...
    def _get_heatmap_single(self, obj_centers2d, obj_bboxes, img_shape, centerness):
        img_h, img_w, _ = img_shape
        ###############################################################################################
        # new_stride1 = math.ceil(math.sqrt(torch.div(img_h * img_w, centerness.shape[0], rounding_mode='trunc'))) # 向下取整
        # print(torch.div(img_h * img_w, centerness.shape[0], rounding_mode='trunc'), new_stride1, 111)
        # new_stride = math.ceil(math.sqrt(torch.ceil(img_h * img_w / torch.tensor(centerness.shape[0])))) + 1# 向上取整
        # print(torch.ceil(img_h * img_w / torch.tensor(centerness.shape[0])), new_stride, 222)
        # print(img_h, img_w, centerness.shape, img_h * img_w / centerness[0])



        # new_stride = int(math.sqrt(torch.div(img_h * img_w, centerness.shape[0], rounding_mode='trunc'))) # 向下取整
        new_stride = int(math.ceil(math.sqrt(torch.div(img_h * img_w, centerness.shape[0], rounding_mode='trunc')))) # 向上取整

        # heatmap = torch.zeros(img_h // new_stride, img_w // new_stride, device=obj_centers2d.device)
        heatmap = torch.zeros(
            math.ceil(img_h / new_stride), 
            math.ceil(img_w / new_stride),
            device=obj_centers2d.device
        ) # 向上取整
        ###############################################################################################
